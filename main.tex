...
On retrouve donc 1 sortie qui indique si l'entrée est une attaque ou non. \newline
...
Pour que FINN fonctionne, il nécessite une quantification binaire codée entre \{-1, +1\}, c'est pourquoi nous avons dû adapter notre modèle Brevitas pour qu'il corresponde à cette contrainte.
...
\begin{itemize}
    ...
    \item Le temps d'éxécution est de 5.8088 ms, sachant que le batch size est de 5000, cela nous donne un temps d'inférence par image de 1.16176 µs, soit 860 758 inférences par seconde. 
...
Il contient 60,000 images d'entraînement et 10,000 images de test, chaque image étant une image en couleur de 32$\times$32 pixels représentant un objet appartenant à l'une des 10 classes suivantes: avion, automobile, oiseau, chat, cerf, chien, grenouille, cheval, bateau, camion. \@

Afin de nous servir de CIFAR10, nous allons utiliser à nouveau le framework FINN en modifiant les paramètres de configuration pour qu'ils soient adaptés au nouveau dataset.
...
En effet, le temps de calcul sur GPU du modèle quantifié est grandement plus élevé que celui du modèle non quantifié, divisant jusqu'à par 3 les performances.\@
...
\begin{figure}[H]
    ...
    \caption{Image du modèle importé dans Netron}
...
Cela semble logique, quand nous voyons la complexité du dataset CIFAR10 comparé à UNSW-NB15.\newline
De même, nous avons vu sur le sceenshot que la vitesse de sortie n'est que de 840 000 FPS pour un MNIST (peut-être pour un UNSW-NB15 car nous n'avons pas l'information du dataset utilisé pour le screenshot). 
...
\caption{Image de la répartition des ressources utilisées}
\end{figure}
Nous voyons en effet que l'utilisation des ressources commence à atteindre certaines limites, ce qui est visible par la grande ligne bleue représentant les DSP.\@
Même si nous sommes loin d'une saturation des ressources autre que DSP, nous voyons que la répartition est homogène à l'ensemble de la carte, ce qui a pour effet de paraitre saturé à tout les plans.\@
